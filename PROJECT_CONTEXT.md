...
- The original protocol-v1 generator serialized questions and images only. Its completed 60-request Qwen3-VL run is therefore retained strictly as an image-only ID-render experiment: it did not send the declared scene table, despite the condition names. Its exact accuracies were raw 0.10, RGB canonical 0.10, no-ID semantics 0.20, ID-render 0.10, ID-render plus crops 0.10, and tagged control 0.20. Prediction SHA-256 remains `d3d02e8213333d05aa351106fb030b309cbe39730a2d2698c5e44b24bd6496fa`; score SHA-256 remains `9a5b62b6e353cb936753af4c1230c712883d13f6ea14bb34b0430ea6d9c83743`.
- Protocol v2 now adds deterministic answer-blind scene context only to the two declared ID conditions. It whitelists scene ID, `S###`, semantics, selected 3D geometry, evidence fields and motion state/evidence. Representative crops are mapped to `S###` through the validated catalog rather than filename inference. A real-package audit found context in all 20 ID requests, none in the other 40 requests, 170 crop image records with no missing ID, and no `ground_truth` token.
- `run_dashscope_mllm_evaluation.py` now writes a required run sidecar binding resumable predictions to the request/package hashes, model, provider/base URL, decoding parameters and protocol version. Missing/mismatched metadata, duplicate IDs and unknown existing IDs are rejected. `score_mllm_evaluation.py` supports an explicitly selected condition subset while retaining exact completeness checks.
- Both affected protocol-v2 conditions were rerun with `qwen3-vl-32b-instruct`, temperature 0 and 64 maximum output tokens. The 20 requests used 244,417 total tokens (244,342 prompt; 75 completion). `raw_plus_3d_only_ids` scored 0.20 exact accuracy and 4.4629 numerical MAE; `raw_plus_3d_ids_and_representative_crops` scored 0.10 and 4.5914. The former answered one of two relative-distance MCQs and the trajectory item; the crop condition answered only the trajectory item.
- Protocol-v2 raw artifacts remain outside Git. Request SHA-256 is `87da229df1d520caf24590b5a1bafea97c7058f8b7ad08203e2c22516b30179d`; predictions `33ad40afc38ea187ddc529270ae226b733c63e993f8d9ed11bb037675dddc672`; run sidecar `83faac5d4673a98542a378640fe477a137ff046ed247d1b40f5e4d597a61560d`; scores `10e19caabe4fc0c6e83169dd82e733c99e85345333b975cb8ae30e67a5990069`.
...
CoTracker inspected and inference-validated on 2026-09-23:
