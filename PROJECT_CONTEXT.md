...
- The Phase-D1 automatic discovery front end is implemented in `reconstruction/detect_video_candidates.py` using an explicitly pinned torchvision Faster R-CNN ResNet-50 FPN v2 COCO V1 checkpoint. It exports frame-local candidate boxes/scores and an optional SAM-2-compatible prompt without modifying source pixels. A direct-script import failure found by the first formal command is fixed and covered by a subprocess test. The complete reconstruction suite passes 59 tests; detector GPU inference validation is still pending.
